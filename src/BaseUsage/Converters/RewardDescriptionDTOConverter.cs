    ...
    internal static class CompetitionParticipationTaskDTOConverter
    {
        public static CompetitionParticipationTaskDTO Convert(this CompetitionParticipationTask task)
        {
            return new CompetitionParticipationTaskDTO(
                task.SessionID,
                (from r in task.Riddles select r.Convert()).ToList()
                );
        }
        public static UserRiddleInfoDTO Convert(this UserRiddleInfo riddleInfo)
        {
            return new UserRiddleInfoDTO(
                riddleInfo.Question,
                (from r in riddleInfo.AvailableAnswers select r.Convert()).ToArray());
        }
        ...
        {
            return new RiddleInfoDTO(info.CompetitionID, info.Question, (from r in info.PossibleAnswers select r.Convert()).ToList(), info.TrueAnswer.Convert(), info.Id);
        }
        public static RiddleInfo Convert(this RiddleInfoDTO info)
        {
            return new RiddleInfo(info.CompetitionID, info.Question, (from r in info.PossibleAnswers select r.Convert()).ToList(), info.TrueAnswer.Convert(), info.ID);
        }
