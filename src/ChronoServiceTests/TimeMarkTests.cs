...
namespace ChronoServiceTests
{
    public class MockSubscriber : ITimeScheduledTaskSubscriber
    {
        ...
        public Task OnRecievedMessage(TimeScheduledTaskData data)
        {
            ...
            Assert.True(delta < ACCEPTABLELATENCY, "Fired too late");
            Recieved = true;
            TriggerCount++;
    ...
    {
        private readonly ITimeScheduler sc;
        ...
        public async Task WaitingIsExactly(int seconds)
        {
            ...
            using var sub = new MockSubscriber(sc, data);
        ...
        {
            using var subscriber = new MockSubscriber(sc);
            for (int i = 1; i < 5; i++)
            {
